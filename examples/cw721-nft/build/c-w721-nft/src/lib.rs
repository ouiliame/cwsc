...
pub mod c_w721_nft {
    ...
    pub fn execute(
      ...
      match msg {
        ExecuteMsg::Mint { token_id, owner, token_uri } =>
          exec_mint_impl(ctx, token_id, owner, token_uri),
        ExecuteMsg::TransferNft { recipient, token_id } =>
          exec_transfer_nft_impl(ctx, recipient, token_id),
  ...
  pub mod implementation {
    ...
    pub fn instantiate_impl(
      ...
      let mut collection = CollectionInfo {
        name: name.clone(),
        symbol: symbol.clone(),
      };
    ...
    pub fn exec_transfer_nft_impl(
      mut ctx: ExecuteCtx,
      recipient: Addr,
      token_id: String
    ) -> Result<Response, ContractError> {
      let mut _response = Response::new();
      let mut token = TOKENS.load(ctx.deps.storage, token_id.clone())?;
    ...
    pub fn exec_burn_impl(
      ...
      _response = _response.add_event(
        Event::new("burn").add_attribute("token_id", format!("{:?}", token_id))
      );
      Ok(_response)
    }
    pub fn query_owner_of_impl(
      mut ctx: QueryCtx,
      token_id: String
    ) -> Result<Binary, ContractError> {
      return Ok(
        to_json_binary(&TOKENS.load(ctx.deps.storage, token_id.clone())?)?
      );
    }
    pub fn query_num_tokens_impl(
      mut ctx: QueryCtx
    ) -> Result<Binary, ContractError> {
      return Ok(to_json_binary(&NUM_TOKENS.load(ctx.deps.storage)?)?);
    }
    pub fn query_collection_info_impl(
      mut ctx: QueryCtx
    ) -> Result<Binary, ContractError> {
