...
pub mod nameservice {
  ...
  pub mod contract {
    ...
    pub fn execute(
      ...
      match msg {
        ...
        ExecuteMsg::Transfer { name, recipient } =>
          exec_transfer_impl(ctx, name, recipient),
      }
    }
    #[cfg_attr(not(feature = "library"), entry_point)]
    pub fn query(
  ...
  pub mod implementation {
    ...
    pub fn instantiate_impl(
      ...
      let mut config = NameserviceConfig {
        purchase_price: purchase_price.clone(),
        transfer_price: transfer_price.clone(),
      };
      CONFIG.save(ctx.deps.storage, &config)?;
      Ok(_response)
    }
    pub fn exec_register_impl(
      mut ctx: ExecuteCtx,
      name: String
    ) -> Result<Response, ContractError> {
      let mut _response = Response::new();
      ...
      let mut record = NameRecord { owner: ctx.info.sender.clone() };
      NAMES.save(ctx.deps.storage, name.clone(), &record)?;
      _response = _response.add_event(
        Event::new("register")
          .add_attribute("name", format!("{:?}", name))
          .add_attribute("owner", format!("{:?}", ctx.info.sender.clone()))
      );
      Ok(_response)
    }
    pub fn exec_transfer_impl(
      ...
      let mut _response = Response::new();
      let mut record = NAMES.load(ctx.deps.storage, name.clone())?;
      ...
      NAMES.save(
        ctx.deps.storage,
        name.clone(),
        &(NameRecord { owner: recipient.clone() })
      )?;
      _response = _response.add_event(
        Event::new("transfer_name")
          .add_attribute("name", format!("{:?}", name))
