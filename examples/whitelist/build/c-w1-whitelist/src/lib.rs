...
pub mod c_w1_whitelist {
  ...
  pub mod implementation {
    ...
    pub fn exec_freeze_impl(
      mut ctx: ExecuteCtx
    ) -> Result<Response, ContractError> {
      let mut _response = Response::new();
      let mut config = ADMIN_CONFIG.load(ctx.deps.storage)?;
      ...
      ADMIN_CONFIG.save(
        ctx.deps.storage,
        &(AdminConfig { owner: config.owner.clone(), frozen: true.clone() })
      )?;
      _response = _response.add_event(
        Event::new("freeze").add_attribute(
          "sender",
    ...
    pub fn exec_add_admin_impl(
      ...
      _response = _response.add_event(
        Event::new("admin_added").add_attribute("admin", format!("{:?}", admin))
      );
      Ok(_response)
    }
    pub fn exec_remove_admin_impl(
      mut ctx: ExecuteCtx,
      admin: Addr
    ) -> Result<Response, ContractError> {
      let mut _response = Response::new();
      ...
      _response = _response.add_event(
        Event::new("admin_removed").add_attribute(
          "admin",
          format!("{:?}", admin)
        )
      );
    ...
    pub fn query_admin_config_impl(
      mut ctx: QueryCtx
    ) -> Result<Binary, ContractError> {
      return Ok(to_json_binary(&ADMIN_CONFIG.load(ctx.deps.storage)?)?);
    }
    pub fn query_is_admin_impl(
      mut ctx: QueryCtx,
      address: Addr
    ) -> Result<Binary, ContractError> {
      return Ok(
        to_json_binary(&ADMINS.load(ctx.deps.storage, address.clone())?)?
      );
    }
  }
  pub mod types {
