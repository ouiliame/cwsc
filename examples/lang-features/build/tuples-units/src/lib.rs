...
pub mod tuples_units {
  pub mod state {
    use cosmwasm_std::*;
    use cw_storage_plus::{ Item, Map };
    use std::collections::HashMap;
    use super::types::*;
    pub const LATEST: Item<TuPair> = Item::new("latest");
  }
  pub mod error {
    use cosmwasm_std::*;
  ...
  pub mod contract {
    ...
    pub fn query(
      ...
      match msg {
        QueryMsg::Latest {} => query_latest_impl(ctx),
      }
    }
  }
  pub mod implementation {
    ...
    pub fn instantiate_impl(
      ...
      let mut p = TuPair(1, String::from("one"));
      let mut s = TuSingle(Uint128::new(100u128));
      let mut m = TuMarker;
      let mut e = TuEmpty;
      LATEST.save(ctx.deps.storage, &p)?;
      Ok(_response)
    }
    pub fn query_latest_impl(
      mut ctx: QueryCtx
    ) -> Result<Binary, ContractError> {
      return Ok(to_json_binary(&LATEST.load(ctx.deps.storage)?)?);
    }
  }
  pub mod types {
    ...
    pub struct TuNone();
    #[cw_serde]
    pub struct TuSingle(pub Uint128);
    #[cw_serde]
    pub struct TuPair(pub i64, pub String);
    #[cw_serde]
    pub struct TuTriple<T>(pub T, pub i64, pub T);
    #[cw_serde]
    pub struct TuMarker;
    #[cw_serde]
    pub struct TuEmpty;
