...
export class FunctionNode implements FunctionStructure {
  ...
  public static readonly Template =
    Handlebars.compile(`pub fn {{name}}({{#args}}{{key}}: {{value}}{{#comma}}, {{/comma}}{{/args}}) -> {{returntype}} {
{{#body}}{{> statement}}{{/body}}}
`);
...
export class ExpressionNode implements ExpressionStructure {
  ...
  public static readonly Template = Handlebars.compile(
    `{{#literal}}{{> literal}}{{/literal}}{{#identifier}}{{> identifier}}{{/identifier}}{{#binary}}{{> binary}}{{/binary}}{{#unary}}{{> unary}}{{/unary}}{{#try}}{{> try}}{{/try}}{{#index}}{{> index}}{{/index}}{{#tuple}}{{> tuple}}{{/tuple}}{{#struct}}{{> struct}}{{/struct}}{{#call}}{{> call}}{{/call}}{{#methodCall}}{{> methodCall}}{{/methodCall}}{{#block}}{{> block}}{{/block}}{{#if_}}{{> if_}}{{/if_}}{{#assign}}{{> assign}}{{/assign}}{{#return}}{{> return}}{{/return}}{{#for}}{{> for}}{{/for}}`.trim()
  );
...
export class BinaryNode implements BinaryStructure {
  ...
  public static readonly Template = Handlebars.compile(
    `({{#left}}{{> expression}}{{/left}}{{{op}}}{{#right}}{{> expression}}{{/right}})`
  );
...
export class UnaryNode implements UnaryStructure {
  ...
  public static from(structure: UnaryStructure): UnaryNode {
    return new UnaryNode(structure.op, ExpressionNode.from(structure.expr));
  }

  public static Template = Handlebars.compile(
    `{{{op}}}{{#expr}}{{> expression}}{{/expr}}`
  );
...
export class TryNode implements TryStructure {
  ...
  public static from(structure: TryStructure): TryNode {
    return new TryNode(ExpressionNode.from(structure.expr));
  }
...
export class IndexNode implements IndexStructure {
  ...
  public static readonly Template = Handlebars.compile(
    `{{#receiver}}{{> expression}}{{/receiver}}[{{#arg}}{{> expression}}{{/arg}}]`
  );
...
export class TupleNode implements TupleStructure {
  constructor(public values: ExpressionNode[]) {}
  ...
  public static readonly Template = Handlebars.compile(
    `({{#values}}{{> expression}}{{#comma}},{{/comma}}{{/values}})`
  );

  public renderHash(): RenderHash<TupleNode> {
    const init = this.values
      .slice(0, this.values.length - 1)
      .map((value) => ({ ...value.renderHash(), comma: true }));
  ...
  public static from(structure: StructStructure): StructNode {
    return new StructNode(
      ...
      structure.fields.map(
        (field) => new PairNode(field.key, ExpressionNode.from(field.value))
      )
    );
  }

  public static readonly Template = Handlebars.compile(
    `{{{name}}}{ {{#fields}}{{{key}}}:{{#value}}{{> expression}}{{/value}}{{#comma}},{{/comma}}{{/fields}} }`
  );

  public renderHash() {
    const init = this.fields
      .slice(0, this.fields.length - 1)
      .map(({ key, value }) => ({
        key,
        value: value.renderHash(),
        comma: true,
      }));
...
export class CallNode implements CallStructure {
  ...
  public static readonly Template = Handlebars.compile(
    `({{#callee}}{{> expression}}{{/callee}})({{#args}}{{> expression}}{{#comma}},{{/comma}}{{/args}})`
  );

  public renderHash(): RenderHash<CallNode> {
    const init = this.args
      .slice(0, this.args.length - 1)
      .map((arg) => ({ ...arg.renderHash(), comma: true }));
...
export class MethodCallNode implements MethodCallStructure {
  ...
  public static readonly Template = Handlebars.compile(
    `({{#callee}}{{> expression}}{{/callee}}).{{{method}}}({{#args}}{{> expression}}{{#comma}},{{/comma}}{{/args}})`
  );
...
export class ReturnNode implements ReturnStructure {
  constructor(public expr: ExpressionNode | undefined) {}
...
export class ForNode {
  ...
  public static from(structure: ForStructure): ForNode {
    return new ForNode(
      new PatternNode(
        structure.binding.identifier
          ? IdentifierNode.from(structure.binding.identifier)
          : undefined,
        structure.binding.tuple
          ? TupleNode.from(structure.binding.tuple)
          : undefined,
        structure.binding.struct
          ? StructNode.from(structure.binding.struct)
          : undefined
      ),
  ...
  public static from(structure: PatternStructure): PatternNode {
    return new PatternNode(
      structure.identifier
        ? IdentifierNode.from(structure.identifier)
        : undefined,
