...
import {
  ...
  CWSEnumType,
  CWSEnumVariant,
  CWSEnumVariantStructType,
  CWSEnumVariantTupleType,
  CWSErrorType,
  CWSEventType,
  CWSExecFnType,
  CWSFnType,
...
export class Contract
  extends ContractDefn
  implements ContractSymbolTable, LexicalSymbolTable
{
  ...
  toplevelType(name: string): CWSType {
    return (
      this._struct.get(name) ??
      this._enum.get(name) ??
      this._tuple.get(name) ??
      (() => {
  ...
  evalToplevelDefn(defn: Defn) {
    ...
    if (defn instanceof StructDefn) {
      ...
      this._struct.set(
        defn.name.value,
        new CWSPlaceholderType(
          () =>
            new CWSStructType(
              defn.name.value,
              defn.fields.map((x) => evalParam(this, x))
            )
        )
      );
      return;
      ...
      this._tuple.set(
        ...
        new CWSPlaceholderType(
          () =>
            new CWSTupleType(
              defn.name.value,
              defn.elements.map((x) => evalType(this, x))
            )
        )
      );
    }
      ...
      this._fn.set(
        ...
        new CWSPlaceholderType(
          () =>
            new CWSFnType(
              defn.fallible,
              (defn.params ?? []).map((x) => evalParam(this, x)),
              defn.returnTy ? evalType(this, defn.returnTy) : new CWSVoidType()
            )
        )
      );
      return;
    ...
    if (defn instanceof InstantiateDefn) {
      ...
      this._instantiate = new CWSPlaceholderType(
        () =>
          new CWSInstantiateFnType(
            (defn.params ?? []).map((x) => evalParam(this, x))
          )
      );
      return;
      ...
      defn.stateFields.forEach((field) => {
        if (field instanceof StateItemDefn) {
          this._state!.fields.push({
            ...
            ty: new CWSPlaceholderType(() => evalType(this, field.ty)),
          });
        }
...
export class BlockScope extends Block implements LexicalSymbolTable {
  ...
  constructor(
    stmts: List<Stmt>,
    public contract: ContractSymbolTable,
    public parent: LexicalSymbolTable
  ) {
    ...
    if (stmt instanceof LetStmt) {
      ...
      if (stmt.binding instanceof TupleBinding) {
        ...
        for (let i = 0; i < stmt.binding.names.length; i++) {
          this._variable.set(
            stmt.binding.names.children[i].value,
            valueTy.elementTys[i]
          );
        }
      }
...
export class Fn extends FnDefn implements LexicalSymbolTable {
  ...
  constructor(
    ...
    (defn.params ?? []).forEach((param) =>
      this._variable.set(
        param.name.value,
        evalType(contract, param.ty ? param.ty : undefined)
      )
    );
...
//#region Expressions
function evalType(
  scope: ContractSymbolTable,
  ty?: TypeExpr | null | undefined
): CWSType {
  if (!ty) {
  ...
  if (ty instanceof StructDefnTypeExpr) {
    return new CWSStructType(
      ty.structDefn.name.value,
      ty.structDefn.fields.map((x) => evalParam(scope, x))
    );
  }

  if (ty instanceof TupleDefnTypeExpr) {
    return new CWSTupleType(
      ty.tupleDefn.name.value,
      ty.tupleDefn.elements.map((x) => evalType(scope, x))
    );
  }
...
function evalEnum(scope: ContractSymbolTable, enm: EnumDefn) {
  return new CWSEnumType(
    enm.name.value,
    enm.variants.map((variant) => {
      if (variant instanceof EnumVariantTupleDefn) {
        return new CWSEnumVariantTupleType(
          variant.name.value,
          (variant.fields ?? []).map((x) => evalType(scope, x))
        );
      }
      if (variant instanceof EnumVariantStructDefn) {
        return new CWSEnumVariantStructType(
          variant.name.value,
          variant.fields.map((x) => evalParam(scope, x))
        );
      }
...
function evalFn(scope: LexicalSymbolTable, defn: FnDefn): CWSFnType {
  ...
  return new CWSFnType(
    false /*TODO*/,
    (fn.params ?? []).map((x) => evalParam(scope.contract, x)),
    returnType
  );
}
...
function evalTupleExpr(scope: LexicalSymbolTable, expr: TupleExpr): CWSType {
  return new CWSTupleType(
    '__Anonymous',
    (expr.exprs ?? []).map((expr) => evalExpr(scope, expr))
  );
}
...
function evalGroupedExpr(
  scope: LexicalSymbolTable,
  expr: GroupedExpr
): CWSType {
...
function evalArithmeticBinOpExpr(
  scope: LexicalSymbolTable,
  expr: BinOpExpr
): CWSType {
...
function evalEqualityBinOpExpr(
  scope: LexicalSymbolTable,
  expr: BinOpExpr
): CWSType {
  const left = evalExpr(scope, expr.lhs);
...
function evalTryCatchElseExpr(
  scope: LexicalSymbolTable,
  expr: TryCatchElseExpr
): CWSType {
  throw new Error('Method not implemented.');
}
